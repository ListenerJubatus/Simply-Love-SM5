...
for pn, menu in pairs(menus) do
	...
	frame[#frame+1]= menu:create_actors{
		x= menu_x[pn], y= 46, width= menu_width, height= menu_height,
	...
	frame[#frame+1]= Def.BitmapText{
		Font= "ScreenPlayerOptions2 explanation", InitCommand= function(self)
			explanations[pn]= self
			self:xy(menu_x[pn] - (menu_width / 2.1), _screen.cy+122)
				:diffuse(ColorLightTone(PlayerColor(pn)))
				:shadowlength(1):wrapwidthpixels(menu_width*1.3):zoom(0.75)
